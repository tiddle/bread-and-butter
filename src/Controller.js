...
class LC {
    ...
    constructor() {
        Logger.info(`butter is loading...`);
        this.load();
        Logger.info(`butter is loaded.`);
    }
    ...
    static cheatStims() {
        ...
        DatabaseServer.tables.globals.config.Health.Effects.Stimulator.Buffs.Buffs_Traning_03 = JSON.parse('[{"BuffType":"SkillRate","Chance":1,"Delay":1,"Duration":3000,"Value":20,"AbsoluteValue":true,"SkillName":"Endurance"},{"BuffType":"SkillRate","Chance":1,"Delay":1,"Duration":3000,"Value":20,"AbsoluteValue":true,"SkillName":"Strength"}]');
        ...
        DatabaseServer.tables.globals.config.Health.Effects.Stimulator.Buffs.Buffs_Traning_04 = JSON.parse('[{"BuffType":"DamageModifier","Chance":1,"Delay":1,"Duration":3000,"Value":-0.5,"AbsoluteValue":true,"SkillName":""},{"BuffType": "HealthRate","Chance": 1,"Delay": 1,"Duration": 3000,"Value": 2,"AbsoluteValue": true,"SkillName": ""}]');
        LC.addAssort(LC.traders.therapist, 'therapist_training_stim_04', 1, null,
            ...
            null);
        DatabaseServer.tables.locales.global.en.templates["training_stim_05"] = JSON.parse('{"Name":"Training stim 05","ShortName":"Training 5","Description":"Training stim of greatly increased endurance, strength and recoil control."}');
    ...
    static fixMarketPrice() {
        ...
        RagfairConfig.dynamic.price.min = LC.config.fix_ragfair.dynamic_price_min;
        RagfairConfig.dynamic.price.max = LC.config.fix_ragfair.dynamic_price_max;
        RagfairConfig.dynamic.condition.min = 1.0;
        RagfairConfig.dynamic.condition.max = 1.0;
        RagfairConfig.dynamic.condition.max = 1.0;
        RagfairConfig.dynamic.batchSize = LC.config.fix_ragfair.batch_size;
        ;
        ;
    ...
    static fixRates() {
        ...
        ;
        DatabaseServer.tables.globals.config.SkillsSettings.SkillProgressRate = LC.config.fix_rates.skill_rate;
    ...
    static fixScavCooldown() {
        ...
        ;
        DatabaseServer.tables.globals.config.SavagePlayCooldown = LC.config.fix_scav_cooldown.cooldown;
    }
        ...
        DatabaseServer.tables.hideout.areas.forEach(area => {
            for (let idx in area.stages) {
                if (area.stages[idx].constructionTime > 0) {
                    area.stages[idx].constructionTime = Math.round(area.stages[idx].constructionTime / LC.config.fix_hideout_speed.multiplier);
                }
            }
        });
            ...
            if (recipe.productionTime > 0) {
                if (LC.config.fix_hideout_speed.exclude_bitcoin_production) {
                    if (recipe._id !== '5d5c205bd582a50d042a3c0e') {
                        recipe.productionTime = Math.round(recipe.productionTime / LC.config.fix_hideout_speed.multiplier);
                    }
                } else {
                    recipe.productionTime = Math.round(recipe.productionTime / LC.config.fix_hideout_speed.multiplier);
                }
...
* Shifting even further drops the success chance very rapidly - so we want to warn against that */
    ...
    static fixWeapons() {
        ...
        if (LC.config.fix_weapons.akm_tlok_support) {
            DatabaseServer.tables.templates.items['59d6088586f774275f37482f']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
            DatabaseServer.tables.templates.items['5a0ec13bfcdbcb00165aa685']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
            DatabaseServer.tables.templates.items['59ff346386f77477562ff5e2']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
            DatabaseServer.tables.templates.items['5abcbc27d8ce8700182eceeb']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
            DatabaseServer.tables.templates.items['5ac66d2e5acfc43b321d4b53']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
            DatabaseServer.tables.templates.items['5ac66d725acfc43b321d4b60']._props.Slots[2]._props.filters[0].Filter.push('5fbc22ccf24b94483f726483');
        }
    }
        ...
        if (sessionID) {
            ...
            if (profile && profile.characters && profile.characters.pmc
                ...
                if (toRecover.length > 0) {
                    let messageContent = {
                        "text": `${toRecover.length} bugged items are recovered by butter.`,
                        "type": 13,
                ...
                if (as_boss) {
                    ...
                    while (slots > 0) {
                        ...
                        for (let idx = 0; idx < 5; idx++) {
                            for (let record = 0; record < LC.config.pmc_conversion_chance.group_size.distribution[idx]; record++) {
                                dist.push(idx + 1);
                            }
                        }
                        let groupSize = dist[LC.getRandomInt(0, dist.length)];
    ...
    static setBotGear(sessionID) {
        ...
        let pmc_rig = [];
        let pmc_bp = [];
        if (LC.config.bot_difficulty.progressive_gear) {
            if (playerLevel < 10) {
